 ...
 * @LastEditTime: 2026-01-19 01:52:16
 */
...
return new class () extends Migration {
    ...
    public function up()
    {
        // 适配多数据库连接配置（和users表保持一致）
        $db_connection = config('youhujun.db_connection');

        // 避免重复创建表
        if (!Schema::connection($db_connection)->hasTable('payments')) {
            Schema::connection($db_connection)->create('payments', function (Blueprint $table) {
            ...
            DB::connection($db_connection)->statement("ALTER TABLE `{$prefix}payments` comment '支付行为主表（记录每笔支付的全量信息，支撑合并支付/分次支付/退款/对账等场景）'");
        }
    }
    ...
    public function down()
    {
        ...
        if (Schema::connection($db_connection)->hasTable('payments')) {
            Schema::connection($db_connection)->dropIfExists('payments');
        }
    }
};
