    ...
    public function up()
    {
        ...
        if (!Schema::connection($dbConnection)->hasTable($this->baseTable)) {
            Schema::connection($dbConnection)->create($this->baseTable, function (Blueprint $table) {
                ...
                $table->char('label_picture_uid', 20)->default('')->comment('标签图片雪花ID(相册图片id)');
                ...
                //索引
                $table->unique(['label_name','deleted_at'], 'uni_labels_name_del');
                $table->unique(['label_code','deleted_at'], 'uni_labels_code_del');
                $table->index('parent_id', 'idx_labels_parent_id');
                $table->index('deep', 'idx_labels_deep');
                $table->index('label_picture_uid', 'idx_labels_pic_uid');
                $table->index('created_time', 'idx_labels_cre_time');
            });
