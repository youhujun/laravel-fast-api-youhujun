...
return new class extends Migration
{
    ...
    public function up()
    {
		...
		if (!Schema::connection($db_connection)->hasTable('user_level_item_union')) 
		{
			Schema::connection($db_connection)->create('user_level_item_union', function (Blueprint $table) 
			{
				$table->id()->comment('主键');
			...
			DB::connection($db_connection)->statement("ALTER TABLE `{$prefix}user_level_item_union` comment '用户级别和配置项关联表'");
		}	
    ...
    public function down()
    {
		...
		if (Schema::connection($db_connection)->hasTable('user_level_item_union')) 
		{
			Schema::connection($db_connection)->dropIfExists('user_level_item_union');
		}
