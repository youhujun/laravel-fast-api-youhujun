    ...
    public function up()
    {
        ...
        if (!Schema::connection($db_connection)->hasTable('user_banks')) {
            Schema::connection($db_connection)->create('user_banks', function (Blueprint $table) {
                ...
                $table->char('user_bank_uid', 20)->notNull()->comment('用户银行卡雪花ID');
                $table->char('user_uid', 20)->notNull()->default('')->comment('用户uid');
                $table->unsignedInteger('bank_id')->notNull()->default(0)->comment('银行id');
                $table->char('bank_front_uid', 20)->notNull()->default('')->comment('银行卡正面(相册图片雪花ID)');
                $table->char('bank_back_uid', 20)->notNull()->default('')->comment('银行卡背面(相册图片雪花ID)');
                ...
                // 索引
                $table->unique('user_bank_uid');
                $table->index('user_uid');
                $table->index('bank_id');
                $table->index('created_time');
                $table->index('bank_front_uid');
                $table->index('bank_back_uid');
