...
return new class extends Migration
{
    ...
    public function up()
    {
		...
		if (!Schema::connection($db_connection)->hasTable('user_info')) 
		{
			Schema::connection($db_connection)->create('user_info', function (Blueprint $table) {
				$table->id()->comment('主键');
			...
			DB::connection($db_connection)->statement("ALTER TABLE `{$prefix}user_info` comment '用户信息表'");
		}
    ...
    public function down()
    {
		...
		if (Schema::connection($db_connection)->hasTable('user_info')) 
		{
			Schema::connection($db_connection)->dropIfExists('user_info');
		}
