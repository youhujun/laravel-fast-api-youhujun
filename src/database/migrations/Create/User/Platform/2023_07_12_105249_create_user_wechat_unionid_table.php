...
return new class extends Migration
{
    ...
    public function up()
    {
		...
		if (!Schema::connection($db_connection)->hasTable('user_wechat_unionid')) 
		{
			Schema::connection($db_connection)->create('user_wechat_unionid', function (Blueprint $table)
			{
				$table->id()->comment('主键');
			...
			DB::connection($db_connection)->statement("ALTER TABLE `{$prefix}user_wechat_unionid` comment '用户微信的unionid'");
		}
    ...
    public function down()
    {
		...
		if (Schema::connection($db_connection)->hasTable('user_wechat_unionid')) 
		{
			Schema::connection($db_connection)->dropIfExists('user_wechat_unionid');
		}
