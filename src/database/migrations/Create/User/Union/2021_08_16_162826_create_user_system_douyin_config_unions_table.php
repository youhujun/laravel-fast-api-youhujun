    ...
    public function up()
    {
        ...
        if (!Schema::connection($db_connection)->hasTable('user_system_douyin_config_unions')) {
            Schema::connection($db_connection)->create('user_system_douyin_config_unions', function (Blueprint $table) {
                $table->id()->comment('主键');
                $table->unsignedBigInteger('user_system_douyin_config_union_uid')->comment('用户抖音配置关联雪花ID');
                ...
                // 索引
                $table->unique('user_system_douyin_config_union_uid', 'uni_sys_douyin_usd_uid');
                $table->unique('openid', 'uni_sys_douyin_openid');
                $table->index('user_uid', 'idx_sys_douyin_user_uid');
                $table->index('created_time', 'idx_sys_douyin_cre_time');
                $table->index('verified_time', 'idx_sys_douyin_ver_time');
                $table->index('system_douyin_config_id', 'idx_sys_douyin_sys_did');
                $table->index('updated_time', 'idx_sys_douyin_upd_time');
            });
