    ...
    public function up()
    {
		...
		if (!Schema::connection($db_connection)->hasTable('user_system_wechat_config_unions'))
		{
			Schema::create('user_system_wechat_config_unions', function (Blueprint $table) {
				...
				// 索引
				$table->unique('user_system_wechat_config_union_uid', 'uni_sys_wechat_usw_uid');
				$table->unique('openid', 'uni_sys_wechat_openid');
				$table->index('user_uid', 'idx_sys_wechat_user_uid');
				$table->index('created_time', 'idx_sys_wechat_cre_time');
				$table->index('type', 'idx_sys_wechat_type');
				$table->index('system_wechat_config_id', 'idx_sys_wechat_sys_wid');
				$table->index('access_token', 'idx_sys_wechat_acc_token');
				$table->index('refresh_token', 'idx_sys_wechat_ref_token');
				$table->index('updated_time', 'idx_sys_wechat_upd_time');
			});
