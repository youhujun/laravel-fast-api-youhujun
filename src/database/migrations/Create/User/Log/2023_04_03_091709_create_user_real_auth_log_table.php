...
return new class extends Migration
{
    ...
    public function up()
    {
		...
		if (!Schema::connection($db_connection)->hasTable('user_real_auth_log')) 
		{
			Schema::connection($db_connection)->create('user_real_auth_log', function (Blueprint $table) 
			{
				$table->id()->comment('主键');
			...
			DB::connection($db_connection)->statement("ALTER TABLE `{$prefix}user_real_auth_log` comment '用户实名认证日志表'");
		}
    ...
    public function down()
    {
		...
		if (Schema::connection($db_connection)->hasTable('user_real_auth_log')) 
		{
			Schema::connection($db_connection)->dropIfExists('user_real_auth_log');
		}
