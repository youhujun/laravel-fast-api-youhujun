    ...
    public function up()
    {
		...
		if (!Schema::connection($db_connection)->hasTable('user_location_logs'))
		{
			Schema::connection($db_connection)->create('user_location_logs', function (Blueprint $table)
			{
				...
				$table->unique('user_location_log_uid', 'uni_user_location_logs_uid');
				$table->index('user_uid', 'idx_user_location_logs_user_uid');
				$table->index('created_time', 'idx_user_location_logs_created_time');
				$table->index('sort', 'idx_user_location_logs_sort');
			});
