    ...
    public function up()
    {
        ...
        if (!Schema::connection($db_connection)->hasTable('user_amount_logs')) {
            Schema::connection($db_connection)->create('user_amount_logs', function (Blueprint $table) {
                $table->char('user_amount_log_uid', 20)->comment('日志uid,雪花ID');
                ...
                $table->unique('user_amount_log_uid', 'uni_user_amount_logs_uid');
                $table->index('user_uid', 'idx_user_amount_logs_user_uid');
                $table->index('created_time', 'idx_user_amount_logs_created_time');
                $table->index('change_type', 'idx_user_amount_logs_change_type');
