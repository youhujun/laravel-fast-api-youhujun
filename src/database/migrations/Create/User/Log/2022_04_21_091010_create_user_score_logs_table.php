        ...
        for ($i = 0; $i < $tableCount; $i++) {
            ...
            if (!Schema::connection($dbConnection)->hasTable($tableName)) {
                Schema::connection($dbConnection)->create($tableName, function (Blueprint $table) use ($i) {
                    ...
                    $table->unique('user_score_log_uid', 'uni_user_score_logs_uid_' . $i);
                    $table->index('user_uid', 'idx_user_score_logs_user_uid_' . $i);
                    $table->index('created_time', 'idx_user_score_logs_created_time_' . $i);
                    $table->index('change_type', 'idx_user_score_logs_change_type_' . $i);
                    $table->index('data_type', 'idx_user_score_logs_data_type_' . $i);
                });
