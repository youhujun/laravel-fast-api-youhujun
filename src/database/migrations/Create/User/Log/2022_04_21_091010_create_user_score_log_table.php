 ...
 * @LastEditTime: 2025-01-07 10:47:59
 */
...
return new class extends Migration
{
    ...
    public function up()
    {
		...
		if (!Schema::connection($db_connection)->hasTable('user_score_log')) 
		{
			Schema::connection($db_connection)->create('user_score_log', function (Blueprint $table) {
			...
			DB::connection($db_connection)->statement("ALTER TABLE `{$prefix}user_score_log` comment '用户积分日志表'");
		}
    }
    ...
    public function down()
    {
		...
		if (Schema::connection($db_connection)->hasTable('user_score_log')) 
		{
			Schema::connection($db_connection)->dropIfExists('user_score_log');
		}
