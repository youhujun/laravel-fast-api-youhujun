...
class CommonLoginFacadeService
{
    ...
    */
    public function clearUserCache($userObject, $token, $provider = 'user', $location = 'phone')
    {
        ...
        Redis::hdel("{$location}_{$provider}:{$provider}", $userObject->biz_id);
        Redis::hdel("{$location}_{$provider}_info:{$provider}_info", $userObject->biz_id);
    }
    ...
    */
    protected function loginCache($userObject, $provider, $location)
    {
        ...
        $tokenResult = Redis::set("{$location}_{$provider}_token:{$userObject->remember_token}", $userObject->biz_id);

        //检测是否有用户信息了
        $hasResult = Redis::hget("{$location}_{$provider}:{$provider}", $userObject->biz_id);
        ...
        $redisResult =  Redis::hset("{$location}_{$provider}:{$provider}", $userObject->biz_id, serialize($userObject));
