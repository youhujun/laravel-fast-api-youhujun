...
class PhoneUserAddressFacadeService
{
   ...
   /**
    ...
    public function addUserAddress($validated,$userObject)
    {
		...
		$userAddressObject->user_uid = $userObject->id;
    ...
    public function updateUserAddress($validated,$userObject)
    {
        ...
        $updateDataArray['revision'] = $userAddressObject ->revision + 1;

        $updateDataArray['updated_time'] = time();

        $updateDataArray['updated_at']  = date('Y-m-d H:i:s',time());

		$updateDataArray['province_id'] = $validated['region_id_array'][0];
		$updateDataArray['region_id'] = $validated['region_id_array'][1];
		$updateDataArray['city_id'] = $validated['region_id_array'][2];

        $updateResult = UserAddress::where($where)->update($updateDataArray);
    ...
    public function deleteUserAddress($validated,$userObject)
    {
        ...
        if($validated['is_delete'])
        {
            $userAddressObject = UserAddress::where('id',$validated['id'])->where('user_uid','=',$userObject->id)->first();
    ...
    public function setDefaultUserAddress($validated,$userObject)
    {
        ...
        if($validated['is_default'])
        {
			...
			$where[] = ['user_uid','=',$userObject->id];
			$updateDataArray = [];
			$updateDataArray['is_default'] = 0;
			$updateOtherResult = UserAddress::where($where)->update($updateDataArray);
	...
	public function getUserDefaultAddress($validated,$userObject)
	{
		...
		$userDefaultAddress = UserAddress::where('user_uid',$userObject->id)->where('is_default',1)->first();
