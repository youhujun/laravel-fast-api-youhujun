<?php
/*
 * @Descripttion: 
 * @version: v1
 * @Author: youhujun 2900976495@qq.com
 * @Date: 2024-07-25 09:47:12
 * @LastEditors: youhujun 2900976495@qq.com
 * @LastEditTime: 2025-06-21 02:45:23
 * @FilePath: \App\Services\Facade\LaravelFastApi\V1\Phone\User\User\PhoneUserFacadeService.php
 */

namespace App\Services\Facade\LaravelFastApi\V1\Phone\User\User;

use Illuminate\Database\Eloquent\Builder;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Storage;
use Illuminate\Support\Facades\Hash;
use Illuminate\Support\Facades\Log;

use App\Exceptions\Phone\CommonException;
use App\Events\Phone\CommonEvent;
use App\Events\LaravelFastApi\V1\Phone\User\UserApplyRealAuthEvent;

use App\Models\LaravelFastApi\V1\User\Log\UserRealAuthLog;

/**
 * @see \App\Facades\LaravelFastApi\V1\Phone\User\User\PhoneUserFacade
 */
class PhoneUserFacadeService
{
   public function test()
   {
       echo "PhoneUserFacadeService test";
   }

    /**
    * 用户申请实名认证
    *
    * @param [type] $validated
    * @param [type] $userObject
    * @return void
    */
   public function realAuthApply($validated,$userObject)
   {
		$result = code(config('phone_code.UserRealAuthApplyError'));

        //先检测用户是否已经认证
        if($userObject->real_auth_status == 20)
        {
            throw new CommonException('UserHasRealAuthApplyError');
        }

		DB::beginTransaction();

        $userApplayRealAuth = new UserRealAuthLog;

		$userApplayRealAuth->user_uid = $userObject->id;
		$userApplayRealAuth->sort = 100;
		$userApplayRealAuth->created_at = time();
		$userApplayRealAuth->created_time = time();
        //状态 申请中
		$userApplayRealAuth->status = 10;
		$userApplayRealAuth->auth_apply_at = time();
		$userApplayRealAuth->auth_apply_time = time();

		$userApplayRealAuthResult = $userApplayRealAuth->save();

		if(!$userApplayRealAuthResult)
		{
			DB::rollBack();
			throw new CommonException('UserRealAuthApplyError');
		}

        UserApplyRealAuthEvent::dispatch($userObject,$validated,1);

        CommonEvent::dispatch($userObject,$validated,'UserRealAuthApply',1);

		DB::commit();

		$result = code(['code'=> 0,'msg'=>'用户申请实名认证成功!']);

		return $result;

   }

}
