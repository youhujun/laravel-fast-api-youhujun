 ...
 * @LastEditTime: 2026-05-01 22:44:54
 * @FilePath: \youhu-laravel-api-12\app\Services\Facade\LaravelFastApi\V1\Phone\Notify\PhonePayNotifyFacadeService.php
 */
...
class PhonePayNotifyFacadeService
{
    ...
    public function wechatJsPayNotify($notifyData)
    {
        ...
        if ($trade_state === 'SUCCESS') {
            ...
            //防止报错,先给默认值
            $defaultAttachArray = ['order' => null, 'goods_uid_array' => null,'user_uid' => null];

            $mergedAttachArray = array_merge($defaultAttachArray, $attachArray);

            ['order' => $orderArray,'goods_uid_array' => $goods_uid_array,'user_uid' => $user_uid] = $mergedAttachArray + [];

            //防止报错,先给默认值
            $defaultOrderArray = ['order_uid' => null,'order_sn' => null];

            $mergedOrderArray = array_merge($defaultOrderArray, $orderArray);

            ['order_uid' => $order_uid,'order_sn' => $order_sn] = $mergedOrderArray + [];
            ...
            $handleParam = ['order_uid' => $order_uid, 'transaction_id' => $transaction_id, 'user_uid' => $user_uid, 'payer_total' => $payer_total];
            //执行支付回调
