...
class PhoneSocketFacadeService
{
	...
	*/
			...
			if(isset($data['send_type']) && !is_null($data['send_type']))
			{
				...
				if($data['send_type'] == 20)
				{
					...
					if (!isset($data['user_uid'])) {
						Log::warning('主动推送失败：缺少 user_uid 参数');
						return;
					}
					$fd = Redis::hget('socket:socket',$data['user_uid']);
					...
					if($fd)
					{
						...
						if ($this->server->isEstablished($fd))
						{
							...
							// 这是一个很好的机会来清理 Redis
							Log::warning("用户 {$data['user_uid']} 的 fd {$fd} 已不存在，正在清理 Redis 记录。");
							Redis::hdel('socket:socket', $data['user_uid']);
						}
					}
					else
					{
						Log::debug(['fd'=>"用户 {$data['user_uid']} 未登录或已断开连接"]);
					}
				}
	...
	private function validateLogin(Server $webSocketServer, Frame $frame, array $data): bool{
		...
		if (!PhoneSocketUserFacade::checkSocketUserIsLogin($data['user_uid'], $data['token'])) {
	...
	protected function handleLogin(Server $webSocketServer, Frame $frame, array $data): void
	{
		...
		//过滤完成,开始处理业务逻辑
		['type'=>$type,'token'=>$token,'user_uid'=>$user_uid] = $data+ [];
		...
		$response = ['code'=>60030,'msg'=>"user_uid-{$user_uid}:socket用户登录失败",'type'=>'error','error'=>'socketUserLoginError'];
		...
		if($checkResult)
		{
			...
			if($saveResult)
			{
				...
				$response = ['code'=>0,'type'=>'login','msg'=>"user_uid-{$user_uid}:socket用户登录成功"];
			}
		}
	...
	protected function handleChat(Server $webSocketServer, Frame $frame, array $data): void
	{
		...
		//过滤完成,开始处理业务逻辑
		['type'=>$type,'token'=>$token,'user_uid'=>$user_uid,'content'=>$content] = $data+ [];
