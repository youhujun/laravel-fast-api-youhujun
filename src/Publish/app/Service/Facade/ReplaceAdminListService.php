 ...
 * @LastEditTime: 2024-08-28 11:14:57
 */
...
use App\Http\Resources\YouHuShop\V1\Admin\Goods\GoodsResource;
use App\Http\Resources\YouHuShop\V1\Admin\Goods\GoodsCollection;
...
class ReplaceAdminListService
{
   ...
   public function importData($path)
   {
    ...
    {
        $result = code(config('admin_code.DefaultReplaceError'));
    ...
    {
        //删除
        $result = code(config('admin_code.RestoreReplaceError'));
    ...
    public function multipleDeleteReplace($validated,$admin)
    {
        ...
        {
            $result = code(config('admin_code.MultipleRestoreReplaceError'));
    ...
    {
        $result = code(config('admin_code.UpdateReplacePropertyError'));
    ...
    public function disableReplace($validated,$admin)
    {
        ...
        $result = code(config('admin_code.AbleReplaceError'));
    ...
    public function multipleDisableReplace($validated,$admin)
    {
        ...
        if(isset($validated['selectId']) && count($validated['selectId']))
        {
            $result = code(config('admin_code.MultipleAbleReplaceError'));
