...
use App\Http\Resources\LaravelFastApi\V1\Admin\Goods\GoodsClassResource;
...
class AdminGoodsClassFacadeService
{
   ...
   /**
    ...
    public function getTreeGoodsClass()
    {
        ...
        {
            $treeGoodsClass = unserialize($redisTreeGoodsClass);
        }
        else
        {
            $treeGoodsClass = $this->getTreeData(['picture']);

            $redisResult = Redis::hset('system:config','treeGoodsClass',serialize($treeGoodsClass));
        }
	...
	public function getSingleGoodsClass($validated,$admin)
	{
		...
		$result = code(['code'=>0,'msg'=>'获取单条分类信息成功','data'=>new GoodsClassResource($goodsClass)]);
