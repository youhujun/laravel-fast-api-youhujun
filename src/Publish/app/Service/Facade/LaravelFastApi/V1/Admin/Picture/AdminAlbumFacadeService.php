...
use App\Http\Resources\LaravelFastApi\V1\Admin\System\Picture\AlbumResource;
use App\Http\Resources\LaravelFastApi\V1\Admin\System\Picture\AlbumCollection;
...
class AdminAlbumFacadeService
{
   ...
   public function getDefaultAlbum($validated,$admin)
   {
        ...
        //必定查询系统相册
        $albumList = $query->where('is_default',1)->orderBy('sort','desc')->orderBy('created_at','asc')->limit(10)->get();
    ...
    public function findAlbum($validated,$admin)
    {
        ...
        $albumList = $query->orderBy('sort','desc')->orderBy('created_at','asc')->get();
 ...
 */
   ...
   public function getAlbumPicture($validated,$admin)
   {
        ...
        $where[] = ['album_id','=',$validated['albumId']];

        $query = AlbumPicture::where($where);

        //查询该相册
        $album = Album::find($validated['albumId']);

		//p($album);die;

        //如果相册类型是管理员相册 并且登录用户只是普通管理员 那么只能查看他自己的所属图片
        if(isDevelop($admin) && isSuper($admin) && $album?->album_type == 10)
        {
            $newWhere = [];
