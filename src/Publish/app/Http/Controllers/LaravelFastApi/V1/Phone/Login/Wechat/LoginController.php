 ...
 * @LastEditTime: 2026-04-30 18:04:25
 * @FilePath: \youhu-laravel-api-12\app\Http\Controllers\LaravelFastApi\V1\Phone\Login\Wechat\LoginController.php
...
use App\DTOs\LaravelFastApi\V1\Phone\User\Login\Wechat\GetOpenIdByCodeWithMiniProgramDTO;
use App\DTOs\LaravelFastApi\V1\Phone\User\Login\Wechat\GetOpenIdByCodeWithMiniGameDTO;
...
class LoginController extends Controller
{
    ...
    public function toGetCodeUrl(Request $request)
    {
        ...
        $requestDTO = (new ToGetCodeUrlDTO())->validate($request->all());

        $result = WechatOfficialFacade::toGetCodeUrl($requestDTO);
    ...
    public function authToLoginByCode(Request $request)
    {
        ...
        $requestDTO = (new AuthToLoginByCodeDTO())->validate($request->all());

        //p($validated);die;
        $collection = WechatOfficialFacade::authToLoginByCode($requestDTO);
    ...
    public function toGetCodeUrlWithLogin(Request $request)
    {
        ...
        if (Gate::forUser($userObject)->allows('phone-user-role')) {
            $requestDTO = (new ToGetCodeUrlWithLoginDTO())->validate($request->all());
    ...
    public function authToLoginByCodeWithLogin(Request $request)
    {
        ...
        if (Gate::forUser($userObject)->allows('phone-user-role')) {
            $requestDTO = (new AuthToLoginByCodeWithLoginDTO())->validate($request->all());
    ...
    public function getOpenIdByCodeWithMiniProgram(Request $request)
    {
        ...
        $collection = WechatLoginFacade::getOpenIdByCodeWithMiniProgram($requestDTO);
