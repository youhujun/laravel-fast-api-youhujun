 ...
 * @LastEditTime: 2025-11-05 22:01:38
 * @FilePath: \app\Http\Controllers\LaravelFastApi\V1\Admin\File\UploadController.php
    ...
    */
    public function uploadConfigFile(Request $request)
    {
        ...
        if(Gate::forUser($admin)->allows('admin-role'))
        {
            ...
            if (!isset($validated['use_type'])) {
                throw new RuleException('RuleRequiredError', 'use_type');
            }
    ...
    public function uploadSinglePicture(Request $request)
    {
        ...
        if(Gate::forUser($admin)->allows('admin-role'))
        {
            ...
            $validator = Validator::make(
                $request->all(),
               [
                    'album_id'=>['bail','nullable',new Numeric],
                    //使用类型 10系统配置 20管理员 30后台
                    'use_type'=>['bail',new Required,new Numeric,new UseType],
                    //文件格式类型 暂时注释不接收
                    'file_type'=>['bail','nullable',new FileType],
