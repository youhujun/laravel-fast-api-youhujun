 ...
 * @LastEditTime: 2025-11-27 09:50:15
 * @FilePath: \app\Http\Resources\LaravelFastApi\V1\Admin\Service\Level\UserLevelResource.php
...
class UserLevelResource extends JsonResource
{
    ...
    public function toArray($request)
    {
		...
		foreach ($userLevelItemUnionCollection as $key => $userLevelItemUnionObject) 
		{
			...
			$itemArray['level_item_id'] = $levelItemObject?->id?$levelItemObject?->id:0;
			$itemArray['item_name'] = $levelItemObject?->item_name?$levelItemObject->item_name:'';
			$itemArray['item_code'] = $levelItemObject?->item_code?$levelItemObject->item_code:'';
			$itemArray['description'] = $levelItemObject?->description?$levelItemObject->description:'';
			$itemArray['value_type'] =  $userLevelItemUnionObject?->value_type?$userLevelItemUnionObject->value_type:0;
			$itemArray['value'] =  $userLevelItemUnionObject?->value?$userLevelItemUnionObject->value:null;
