<?php
/*
 * @Description:
 * @version: v1
 * @Author: youhujun youhu8888@163.com & xueer
 * @Date: 2026-04-08 08:00:49
 * @LastEditors: youhujun youhu8888@163.com & xueer
 * @LastEditTime: 2026-05-10 12:22:52
 * @FilePath: \youhu-laravel-api-12\app\DTOs\LaravelFastApi\V1\Admin\Service\Level\UserLevel\UpdateUserLevelDTO.php
 * Copyright (C) 2026 youhujun & xueer . All rights reserved.
 */
namespace App\DTOs\LaravelFastApi\V1\Admin\Service\Level\UserLevel;

use App\DTOs\Traits\BaseDTOTrait;
use Illuminate\Support\Facades\Validator;
use App\Exceptions\Common\RuleException;
use App\Rules\Pub\Required;
use App\Rules\Pub\Numeric;
use App\Rules\Pub\CheckUnique;
use App\Rules\Pub\CheckString;
use App\Rules\Pub\CheckBetween;
use App\Rules\Pub\CheckArray;
use App\Rules\Pub\FormatTime;
use App\Rules\Pub\ChineseCodeNumberLine;
use App\Rules\Pub\CheckDbUnique;
/**
 * @see \App\Http\Controllers\LaravelFastApi\V1\Admin\Service\Level\UserLevelController
 */
class UpdateUserLevelDTO
{
    use BaseDTOTrait;
    public $ignore_id;
    // 字段定义（赋默认值）
    public string $id = '';
    public string $sort = '';
    public string $level_name = '';
    public string $level_code = '';
    public ?string $amount = null;
    public ?string $background_picture_uid = null;
    public ?string $note = null;
    function __construct($ignore_id = 0)
    {
        $this->ignore_id = $ignore_id;
    }

    // ========== 核心约定：字段映射（必实现） ==========
    /**
     * 字段映射（供TS类型生成/微服务对接，支持单表/多表格式）
     * 多表格式：['表名' => ['字段1','字段2'], ...]
     * 单表格式：['字段1','字段2']（极简适配）
     * @return array
     */
    public function getFieldMap(): array
    {
        return ['id', 'sort', 'level_name', 'level_code', 'amount', 'background_picture_uid', 'note'];
    }

    // ========== 按需扩展：校验规则（非必实现，复用场景才加） ==========
    /**
     * 校验规则（仅需要复用校验时实现，否则保留空数组）
     * 已写好的接口：无需修改，默认空数组即可
     * 复用场景：复制控制器校验规则到此处，实现跨模块复用
     * @return array
     */
    public function rules(): array
    {
        return [
            'id' => ['bail', new Required(), new Numeric()],
            'sort' => ['bail', new Required(), new Numeric()],
            'level_name' => ['bail', new Required(), new CheckString(), new CheckBetween(1, 30), new ChineseCodeNumberLine(), new CheckDbUnique('user_levels', 'level_name', $this->ignore_id)],
            'level_code' => ['bail', new Required(), new CheckString(), new CheckBetween(1, 30), new ChineseCodeNumberLine(), new CheckDbUnique('user_levels', 'level_code', $this->ignore_id)],
            'amount' => ['bail', 'nullable', new Numeric()],
            'background_picture_uid' => ['bail', 'nullable', new Numeric()],
            'note' => ['bail', 'nullable', new CheckString(), new CheckBetween(1, 60)],
        ];
    }

    /**
     * 核心：执行校验并抛出异常（把控制器的校验逻辑移到这里）
     * @param array $data 待校验的数据（如request->all()）
     * @return self 校验通过返回DTO实例（已赋值）
     * @throws RuleException 校验不通过直接抛异常
     */
    public function validate(array $data): self
    {
        // 1. 初始化Validator（复用DTO的rules）
        $validator = Validator::make($data, $this->rules(), []);
        // 2. 执行校验，获取校验后的数据
        $validated = $validator->validated();

        // 3. 校验字段是否存在（和你控制器里的逻辑一致）
        $requiredFields = ['id', 'sort', 'level_name', 'level_code'];
        foreach ($requiredFields as $field) {
            if (!isset($validated[$field])) {
                throw new RuleException('RuleRequiredError', $field);
            }
        }

        // 4. 校验通过，给DTO赋值并格式化
        $this->fill($validated); // 复用BaseDTOTrait的fill方法
        $this->formatFields(); // 执行字段格式化

        return $this;
    }

    /**
     * 字段格式化（按需实现，非必填）
     * 说明：Laravel Request已默认trim字符串，仅以下场景需实现：
     * 1. 时间戳格式转换（如string转int、时间字符串转Carbon）
     * 2. 类型强转（如string转int/float）
     * 3. 特殊字符过滤（如防XSS、替换非法字符）
     * 4. 业务规则格式化（如手机号去空格/横杠）
     * @return self
     */
    public function formatFields(): self
    {
        // 按需添加格式化逻辑，默认留空即可
        // 示例：
        // $this->timestamp = (int)$this->timestamp; // 时间戳转整型
        // $this->mobile = preg_replace('/\D/', '', $this->mobile); // 手机号去非数字
		// 统一调用自定义格式化方法 f() 作用是过滤HTML标签/转义实体

        foreach ($this->getFieldMap() as $field) {
        	$this->$field = f($this->$field);
    	}
        return $this;
    }
}
