 ...
 * @Date: 2026-04-07 14:30:47
 * @LastEditors: youhujun youhu8888@163.com & xueer
 * @LastEditTime: 2026-05-19 01:05:49
 * @FilePath: \youhu-laravel-api-12\app\DTOs\LaravelFastApi\V1\Admin\User\Admin\Administrator\UpdateAdminDTO.php
 * Copyright (C) 2026 youhujun & xueer . All rights reserved.
 */

namespace App\DTOs\LaravelFastApi\V1\Admin\User\Admin\Administrator;
...
class UpdateAdminDTO
{
    ...
    public string $phone = '';
    public array $role_cascader_id_array = [];
    ...
    {
        return ['admin_uid', 'phone', 'role_cascader_id_array'];
    }
    ...
    public function rules(): array
    {
        return [
            ...
            'admin_uid' => [new Required(), new Numeric()],
            'phone' => ['bail', new Required(), new CheckString(), new Phone()],
            'role_cascader_id_array' => ['bail', 'nullable', new CheckArray()],
        ];
    }
    ...
    {
        // 1. 获取id用于动态校验
        $admin_uid = check_id($data['admin_uid'] ?? 0);

        // 3. 初始化Validator（复用DTO的rules）
        $validator = Validator::make($data, $this->rules(), []);
        // 4. 动态添加唯一性校验
        $validator->after(function ($validator) use ($admin_uid) {
            $validator->sometimes('phone', new CheckUnique('admins', 'phone', 'user','user_uid', $admin_uid), function ($query) {
                return true;
            });
        });
        ...
        // 6. 校验字段是否存在（和你控制器里的逻辑一致）
        $requiredFields = ['admin_uid','phone'];
