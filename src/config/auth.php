...
if(!function_exists('getAdminRoles'))
{
    ...
    function getAdminRoles(Admin $admin)
    {
        ...
        // 3. 先判断admin->user_id是否为空，避免无效查询
        if(empty($admin->user_id) || empty($admin->id)) {
            return $rolesArray;
        }

        // 获取Redis缓存
        $redisRoles = Redis::hget($redisKey, $admin->id);
        if($redisRoles)
        {
        ...
        else
        {
            ...
            if(!empty($roleIdArray))
            {
                ...
                if(!empty($rolesArray))
                {
                    $result = Redis::hset($redisKey, $admin->id, json_encode($rolesArray));
